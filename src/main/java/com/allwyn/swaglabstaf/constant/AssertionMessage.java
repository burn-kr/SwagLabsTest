package com.allwyn.swaglabstaf.constant;

public class AssertionMessage {

    // page
    public static final String PAGE_URL_INCORRECT = "Page URL is not as expected";
    public static final String PAGE_TITLE_INCORRECT = "The page title is not as expected";

    // error message
    public static final String ERROR_MESSAGE_INCORRECT = "The error message is not as expected";

    // list, numbers
    public static final String ELEM_LIST_INCORRECT = "The list of elements is not as expected";
    public static final String CART_ITEMS_NUMBER_INCORRECT = "The number of checkout cart items in no as expected";

    // controls
    public static final String BADGE_VALUE_INCORRECT = "The shopping cart badge value is not as expected";
    public static final String BUTTON_TEXT_INCORRECT = "The button text is not as expected";
    public static final String BUTTON_STATE_INCORRECT = "The enabled state of the button is incorrect";
}
